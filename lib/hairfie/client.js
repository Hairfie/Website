...
Client.prototype.getSimilarBusinesses = function (businessId, limit, config) {
    var req = request.get(this._buildUrl('businesses/'+businessId+'/similar?limit='+limit));
...
Client.prototype.getHairfieSearchResult = function (query, config) {
    var req = request
        .get(this._buildUrl('hairfies/search'))
        .query(_.toQuery(query));
...
function pictureToApiValue(picture) {
    return _.isObject(picture) && (picture.id || picture.url) || picture;
}
