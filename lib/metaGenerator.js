...
var urlGenerator = {
    main: function() {
        return serverConfig.URL + '/fr';
    },
    user: function(user) {
        return serverConfig.URL + '/fr' + '/users/' + user.id;
    },
    business: function(business) {
        if(business.slug) {
            return serverConfig.URL + '/fr' + '/businesses/' + business.id + '/' + business.slug;
        } else {
            return serverConfig.URL + '/fr' + '/businesses/' + business.id;
        }

    },
    hairfie: function(hairfie) {
        return serverConfig.URL + '/fr' + '/hairfies/' + hairfie.id;
    }
}
...
module.exports = {
    getBasicMetadata: function() {
        ...
        metas.push({property: "og:image", content: serverConfig.URL + '/img/background-pro.jpg'});
        metas.push({property: "og:title", content: "Hairfie"});
        metas.push({property: "og:url", content: urlGenerator.main()});
        metas.push({property: "og:description", content: "Trouvez rapidement le coiffeur qui vous correspond près de chez vous et prenez rendez-vous gratuitement en ligne en quelques clics."})
    ...
    getHairfieMetadata: function (hairfie) {
        ...
        if(hairfie.hairdresser) {
            ...
            description = 'Réservez gratuitement en ligne la même prestation réalisée au salon ' + business.name + ' sur Hairfie';
        } else {
        ...
        if (caption)        metas.push({property: 'og:caption', content: caption})
        if (authorUrl)      metas.push({property: "hairfie:author", content: authorUrl});
        if (businessUrl)    metas.push({property: "hairfie:business", content: businessUrl});
        if (hairdresserUrl) metas.push({property: "hairfie:hairdresser", content: hairdresserUrl});
    ...
    getBusinessMetadata: function(business) {
        ...
        if(business.pictures) {
            ...
            metas.push({property: "og:image", content: serverConfig.URL + '/images/logo-red.png'});
        }

        return metas;
    },
    getHomeProMetadata: function() {
        var metas = [];
        metas.push.apply(metas, globalMetas);
        metas.push({property: "og:image", content: serverConfig.URL + '/img/background-pro-trans.jpg'});
        metas.push({property: "og:title", content: "Hairfie pour les coiffeurs"});
        metas.push({property: "og:url", content: urlGenerator.main()});
        metas.push({property: "og:description", content: "Ajoutez gratuitement votre salon en 3 clics et commencez à diffuser votre talent"})
