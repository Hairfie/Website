...
function searchToTitle(search, place, tab, categories) {
    var title = '';

    if (search.categories && search.categories.length == 1 && place && place.title && place.title.categories && place.title.categories[search.categories[0]]) {
        title = place.title.categories[search.categories[0]];
    } else if (search.tags && search.tags.length == 1 && place && place.title && place.title.tags && place.title.tags[search.tags[0]]) {
        title = place.title.tags[search.tags[0]];
    } else if(_.isUndefined(search.categories) && _.isUndefined(search.tags) && tab == 'business' && place && place.title && place.title.default){
        title = place.title.default;
    }
    else if(_.isUndefined(search.tags) && _.isUndefined(search.categories) && tab == 'hairfie' && place && place.title && place.title.tags && place.title.tags.default){
        title = place.title.tags.default;
    } else {
        if (tab == 'business') {
...
function searchToDescription (search, place) {
    if (search.categories && search.categories.length == 1 && place && place.description && place.description.categories && place.description.categories[search.categories[0]]) {
