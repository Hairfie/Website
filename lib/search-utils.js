...
function searchToTitle(search, place, tab, categories) {
    ...
    if (search.categories && search.categories.length == 1 && place.title && place.title.categories && place.title.categories[search.categories[0]]) {
    ...
    } else if(_.isUndefined(search.categories) && _.isUndefined(search.tags) && tab == 'business' && place.title && place.title.default){
        title = place.title.default;
    }
    else if(_.isUndefined(search.tags) && _.isUndefined(search.categories) && tab == 'hairfie' && place.title && place.title.tags && place.title.tags.default){
        ...
        if(place.name) title = title + ' - ' + (place.name).split(',')[0]
    }
    return title || '';
}
